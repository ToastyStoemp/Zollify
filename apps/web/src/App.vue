...
import { currentAccount, pendingCount, syncNow, syncState, toasts } from '@zollify/platform';
...
.sidebar {
  ...
  background: var(--zfy-surface, #fff); border-right: 1px solid var(--zfy-line, #d6dde4);
}
.brand { font-weight: 800; font-size: 1.25rem; letter-spacing: -.02em; }
.brand span { color: var(--zfy-accent, #0e7c66); }
...
nav a:hover { background: var(--zfy-surface-2, #e9edf1); }
nav a.router-link-active { background: var(--zfy-accent-soft, #deeee9); color: var(--zfy-accent-ink, #0a5a4a); font-weight: 600; }
hr { border: 0; border-top: 1px solid var(--zfy-line, #d6dde4); margin: .5rem 0; width: 100%; }
.sync { margin-top: auto; display: flex; align-items: center; gap: .45rem; font-size: .8rem; justify-content: flex-start; }
.sync .dot { width: .5rem; height: .5rem; border-radius: 50%; background: var(--zfy-accent, #0e7c66); }
.sync .dot.offline { background: var(--zfy-muted, #5a6472); }
.sync .dot.error { background: var(--zfy-danger, #c6512f); }
.sync .dot.syncing { background: var(--zfy-warning, #c08a2e); }
.sync .pending { color: var(--zfy-muted, #5a6472); font-variant-numeric: tabular-nums; }
.who { margin-top: .5rem; font-size: .8rem; color: var(--zfy-muted, #5a6472); }
.who .name { font-weight: 600; color: var(--zfy-ink, #141a22); }
.content { padding: 1.5rem; }
.toasts { position: fixed; right: 1rem; bottom: 1rem; display: flex; flex-direction: column; gap: .5rem; }
.toast { margin: 0; padding: .6rem .9rem; border-radius: 8px; background: var(--zfy-surface, #fff); border: 1px solid var(--zfy-line, #d6dde4); box-shadow: 0 8px 24px -14px rgba(20,26,34,.4); }
.toast.error { border-color: var(--zfy-danger, #c6512f); }
.toast.success { border-color: var(--zfy-accent, #0e7c66); }
