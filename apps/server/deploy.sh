...
# Every compose call takes the same files; the Caddy overlay joins in when
# CADDY_NETWORK is set (Caddy itself running in Docker on this host).
compose_args=(-f "$compose_file" --env-file "$env_file")
if grep -qE '^CADDY_NETWORK=.+' "$env_file" 2>/dev/null; then
  compose_args+=(-f apps/server/docker-compose.caddy.yml)
fi
