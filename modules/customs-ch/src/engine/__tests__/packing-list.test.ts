...
describe('packing list - no Tariff Rate / VAT Rate columns', () => {
  it('never shows a rate percentage, but keeps the HS/tariff number', () => {
    const html = buildPackingListHtml(state([product({ tariffNo: '4911.91.00', tariffRate: 8.1, vatRate: 8.1 })]), 'detailed');
...
describe('packing list - art-print artist attribution', () => {
  it('shows "Title (Year) - Artist"', () => {
    const html = buildPackingListHtml(state([product({ id: 'a', title: 'Sunset', type: 'Art Print', year: 2024 })]), 'detailed');
    expect(html).toContain('Sunset (2024) - Jane Doe');
  });

  it('leaves non-art-print titles untouched', () => {
    const html = buildPackingListHtml(state([product({ title: 'Enamel Pin', type: 'Pin' })]), 'detailed');
    expect(html).toContain('Enamel Pin');
    expect(html).not.toContain('Enamel Pin -');
  });
});

describe('packing list - excludes zero-stock products', () => {
  ...
  it('keeps a product whose stock lives only in its variants', () => {
    ...
    const html = buildPackingListHtml(state([p]), 'detailed');
...
describe('packing list - by-type grouping', () => {
