...
<template>
  <section class="page customs-index">
    <header><h1>Customs (Germany)</h1></header>
    <p class="lede">
      Export and re-import preparation for the ATLAS side of the trip: packing lists and a filing
      checklist, built from an event's claimed stock and its sales. Filing ATLAS itself is still done
      by your declarant or customs broker - this module doesn't have verified ATLAS access.
    </p>
        ...
        <template v-if="group.list.length">
          ...
          <ul class="events">
            <li v-for="event in group.list" :key="event.id">
              <router-link :to="{ name: 'customs-de:documents', params: { eventId: event.id } }">
                <span class="name">
                  {{ event.name }}
